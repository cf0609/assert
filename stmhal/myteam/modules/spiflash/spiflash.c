...
static bool spiflash_wait_op_finish(void)
{
	...
	timeout = HAL_SPI_Transmit(spiflash->phspi, &spiflash->current_flash->read_sr, 1, TIMEOUT) == HAL_OK?TIMEOUT:0;
	do{
...
/**
 * @brief spi flash earse chip
 */
void spiflash_chip (void)
{
	uint8_t flash_status = 0xC7;
	FLASH_CS_ENABLE();
	HAL_SPI_Transmit(spiflash->phspi, &flash_status, 1, TIMEOUT);
	FLASH_CS_DISABLE();
...
{
#ifdef SPIFLASH_SUPPORT_ONLYONE
	if( spiflash->current_flash ){
		/* check flash id */
...
const uint8_t strings[] = "hello MPYB\nTest SPI FLASH Successful\nAuthor Longqi\n";
...
static bool spiflash_write_enable(void)
{
	...
	FLASH_CS_ENABLE();
	ret_status = HAL_SPI_Transmit(spiflash->phspi, &spiflash->current_flash->write_en, 1, TIMEOUT) == HAL_OK?true:false;
	FLASH_CS_DISABLE();
...
static bool spiflash_erase_a_sector(uint32_t flash_addr)
{
	...
	FLASH_CS_ENABLE();
    HAL_SPI_Transmit(spiflash->phspi, &spiflash->current_flash->erase_sec, 1, TIMEOUT);
    HAL_SPI_Transmit(spiflash->phspi, &array[1], 3, TIMEOUT);
	FLASH_CS_DISABLE();
...
uint32_t spiflash_get_sector_info(uint32_t flash_addr, uint32_t *psector_start)
{
	uint32_t sector_index = flash_addr / spiflash->current_flash->sectorsize;
	*psector_start = sector_index * spiflash->current_flash->sectorsize;
	return sector_index;
}

uint32_t spiflash_conver_addr_by_sector(uint32_t *pflash_addr, uint32_t sector_start)
{
	*pflash_addr = sector_start * spiflash->current_flash->sectorsize;
	return sector_start;
}
/**
...
static uint32_t spiflash_write_a_page (uint32_t flash_addr, uint8_t *pbuffer, uint32_t num_of_bytes)
{
	...
	/* array[0] = parray[3]; */ array[1] = parray[2]; array[2] = parray[1]; array[3] = parray[0];
	FLASH_CS_ENABLE();
    HAL_SPI_Transmit(spiflash->phspi, &spiflash->current_flash->write_cmd, 1, TIMEOUT);
    HAL_SPI_Transmit(spiflash->phspi, &array[1], 3, TIMEOUT);
    HAL_SPI_Transmit(spiflash->phspi, pbuffer, num_of_bytes, TIMEOUT);
...
uint32_t spiflash_read_buffer (uint32_t flash_addr, uint8_t *pbuffer, uint32_t num_of_bytes)
{
	...
	/* array[0] = parray[3]; */ array[1] = parray[2]; array[2] = parray[1]; array[3] = parray[0];
	FLASH_CS_ENABLE();
    HAL_SPI_Transmit(spiflash->phspi, &spiflash->current_flash->read_cmd, 1, TIMEOUT);
    HAL_SPI_Transmit(spiflash->phspi, &array[1], 3, TIMEOUT);
